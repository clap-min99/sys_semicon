...
void led_distance_control(int dist_cm);
...
volatile int echo_done = 0;  // 하강 엣지 감지 플래그
...
void make_trigger(void) 
{
	...
	_delay_us(15);					// 규격에는 10µs인데 redundancy로 
	TRIG_PORT &= ~(1 << TRIG_PIN);	// low로 만든다.
...
void ultrasonic_processing(void)
{
	if (echo_done == 1)	// 1초
	{
		...
		int dist_cm = ultrasonic_distance / 58;
		led_distance_control(dist_cm);
	}
...
{
	if (dist_cm <= 2)
	PORTA = 0b00000001;  // led 0
	else if (dist_cm <= 3)
	PORTA = 0b00000011;  // led 0,1
	else if (dist_cm <= 4)
	PORTA = 0b00000111;  // led 0,1,2
	else if (dist_cm <= 5)
	PORTA = 0b00001111;  // led 0,1,2,3
	else if (dist_cm <= 6)
	PORTA = 0b00011111;  // led 0,1,2,3,4
	else if (dist_cm <= 7)
	PORTA = 0b00111111;  // led 0,1,2,3,4,5
	else if (dist_cm <= 10)
	PORTA = 0b01111111;  // led 0,1,2,3,4,5,6
	else
	PORTA = 0b11111111;  // led 0,1,2,3,4,5,6,7
}
